...
#include "std_msgs/Int32.h" //Added by Xyneef
...
public:
    ...
    ros::Publisher FilterMode_pub; //Added by Xyneef

    // Service
    ros::ServiceServer autocalibration_serv;
    ros::ServiceServer gyrocalibration_serv;
    ros::ServiceServer resetHeading_serv;
    // Services Added by Xyneef
    ros::ServiceServer resetObject_serv;
    ros::ServiceServer resetAlignment_serv;
    ros::ServiceServer setFilterModeGyro_serv;
    ros::ServiceServer setFilterModeGyroAcc_serv;
    ros::ServiceServer setFilterModeGyroAccMag_serv;
    ros::ServiceServer setFilterModeGyroAccDCM_serv;
    ros::ServiceServer setFilterModeGyroAccMagDCM_serv;
        ...
        })
    {
        ...
        private_nh.param<std::string>("frame_id", frame_id, "ZenIMU");  //Frame name modified by Xyneef
        ...
        // Filter Mode publisher added by Xyneef
        FilterMode_pub = nh.advertise<std_msgs::Int32>("filter_mode",1,true);
        ...
        // Services added by Xyneef
        resetObject_serv= nh.advertiseService("reset_object", &OpenZenSensor::resetObject,this);
        resetAlignment_serv = nh.advertiseService("reset_alignment", &OpenZenSensor::resetAlignment,this);
        setFilterModeGyro_serv = nh.advertiseService("filter_mode_gyro", &OpenZenSensor::setFilterModeGyro,this);
        setFilterModeGyroAcc_serv =nh.advertiseService("filter_mode_gyro_acc", &OpenZenSensor::setFilterModeGyro_Acc, this); 
        setFilterModeGyroAccMag_serv = nh.advertiseService("filter_mode_gyro_acc_mag", &OpenZenSensor::setFilterModeGyro_Acc_Mag, this); 
        setFilterModeGyroAccDCM_serv = nh.advertiseService("filter_mode_gyro_acc_DCM", &OpenZenSensor::setFilterModeGyro_Acc_DCM, this); 
        setFilterModeGyroAccMagDCM_serv = nh.advertiseService("filter_mode_gyro_acc_mag_DCM", &OpenZenSensor::setFilterModeGyro_Acc_Mag_DCM, this); 
    ...
    bool run(void)
    {
        ...
        if (!hasImu)
        {
            ...
            // Publishing the Filter Mode as a Node
            // Added by Xyneef
            publishFilterMode();
        }
        ...
        m_sensorThread.start( SensorThreadParams{
            ...
            mag_pub,
            nav_pub,
            FilterMode_pub, //Added by Xyneef. Note by author: not sure whether this is actually necessary
    ...
    void publishFilterMode()
    {
        ...
        if (!m_zenImu){
            ROS_INFO("No IMU component available, can't publish filter mode");
            return;
        }

        auto resPair = m_zenImu->getInt32Property(ZenImuProperty_FilterMode);
        auto error = resPair.first;
        auto FilterMode = resPair.second;
        if (error)
        {
            ROS_INFO("Error: can't get filter mode");
        }
        else
        {
            msg.data = FilterMode;
            FilterMode_pub.publish(msg);
        }
    }
    ...
    // Service to reset the Object
    // Added by Xyneef
    bool resetObject(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU compontent available, can't reset object");
            return false;
        }

        ROS_INFO("reset_object");
        // Offset reset parameters:
        // 0: Object reset
        // 1: Heading reset
        // 2: Alignment reset
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_OrientationOffsetMode, 0)) 
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Object reset";
        } 
        else 
        {
            res.success = true;
            res.message = "[Success] Object reset";
        }
        return res.success;

    }
    // Service to reset the Alignment
    // Added by Xyneef
        bool resetAlignment(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU compontent available, can't reset alignment");
            return false;
        }

        ROS_INFO("reset_alignment");
        // Offset reset parameters:
        // 0: Object reset
        // 1: Heading reset
        // 2: Alignment reset
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_OrientationOffsetMode, 2)) 
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Alignment reset";
        } 
        else 
        {
            res.success = true;
            res.message = "[Success] Alignment reset";
        }
        return res.success;
    }

    // Service to Set the Filter Mode to GYRO only (0)
    // Added by Xyneef

    bool setFilterModeGyro(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU component available, can't set filter mode");
            return false;
        }
        
        ROS_INFO("Set Filter Mode to Gyro (0)");
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_FilterMode,0))
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Filter Mode Not Set";
        }
        else
        {
            res.success = true;
            res.message = "[Success] Filter Mode Set";
        }
        publishFilterMode();
        return res.success;
    }
    // Service to Set the Filter Mode to GYRO+ ACC(1)
    // Added by Xyneef 
    bool setFilterModeGyro_Acc(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU component available, can't set filter mode");
            return false;
        ...
        ROS_INFO("Set Filter Mode to Gyro+Acc (1)");
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_FilterMode,1))
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Filter Mode Not Set";
    ...
    // Service to Set the Filter Mode to GYRO+ACC+MAG (2)
    // Added by Xyneef
    bool setFilterModeGyro_Acc_Mag(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU component available, can't set filter mode");
            return false;
        ...
        ROS_INFO("Set Filter Mode to Gyro+Acc+Mag (2)");
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_FilterMode,2))
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Filter Mode Not Set";
    ...
    // Service to Set the Filter Mode to GYRO+ACC (DCM) (3)
    // Added by Xyneef
    bool setFilterModeGyro_Acc_DCM(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU component available, can't set filter mode");
            return false;
        ...
        ROS_INFO("Set Filter Mode to Gyro+Acc (DCM) (3)");
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_FilterMode,3))
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Filter Mode Not Set";
    ...
    // Service to Set the Filter Mode to GYRO+ACC+MAG (DCM) (4)
    // Added by Xyneef
    bool setFilterModeGyro_Acc_Mag_DCM(std_srvs::Trigger::Request &req, std_srvs::Trigger::Response &res)
    {
        if (!m_zenImu) {
            ROS_INFO("No IMU component available, can't set filter mode");
            return false;
        ...
        ROS_INFO("Set Filter Mode to Gyro+Acc+Mag(DCM) (4)");
        if (auto error = m_zenImu->setInt32Property( ZenImuProperty_FilterMode,4))
        {
            ROS_INFO("Error");
            res.success = false;
            res.message = "[Failed] Filter Mode Not Set";
 ...
 private:
    ...
    struct SensorThreadParams
    {
        ...
        ros::Publisher & mag_pub;
        ros::Publisher & nav_pub;
        ros::Publisher & FilterMode_pub;  //Added by Xyneef. Note by author: not sure whether this is actually necessary
